...
function Shell({ client, session }: { client: SupabaseClient; session: Session }) {
  ...
  return <div className="admin-shell">
    ...
    <aside className={menuOpen ? 'admin-sidebar is-open' : 'admin-sidebar'}>
      ...
      <nav aria-label="Administration">{(['Innehåll', 'Webbplats'] as const).map((group) => <div className="admin-nav-group" key={group}><span>{group}</span>{tabs.filter((item) => item.group === group).map((item) => <button type="button" className={tab === item.key ? 'is-active' : ''} aria-current={tab === item.key ? 'page' : undefined} key={item.key} onClick={() => navigate(item.key)}><i className={`ph ${item.icon}`} aria-hidden="true" /><span>{item.label}</span></button>)}</div>)}</nav>
    ...
    <main id="admin-main">{tab === 'dashboard' && <DeploymentDashboard client={client} onNavigate={navigate} />}{tab === 'pages' && <PagesManager client={client} />}{tab === 'news' && <ResourceEditor client={client} kind="news_posts" />}{tab === 'calendar' && <ResourceEditor client={client} kind="calendar_events" />}{tab === 'documents' && <ResourceEditor client={client} kind="documents" />}{tab === 'media' && <MediaManager client={client} />}{tab === 'navigation' && <NavigationManager client={client} />}{tab === 'settings' && <SettingsManager client={client} />}{tab === 'revisions' && <RevisionHistory client={client} />}</main>
  </div>;
}
...
export default function AdminApp() {
  ...
  return <AdminDirtyProvider><Shell client={client} session={session} /></AdminDirtyProvider>;
}
