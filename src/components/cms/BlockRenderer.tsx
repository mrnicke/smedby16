...
import { contentBlockSchema, type ContentBlock, type EditorDocumentV2 } from '../../lib/cms/schema';
...
type Props = { blocks?: ContentBlock[]; document?: EditorDocumentV2 | null; preview?: boolean; collections?: PublicCollections; components?: Record<string,{published_definition:EditorDocumentV2}> };
...
export default function BlockRenderer({ blocks = [], document, preview = false, collections, components = {} }: Props) {
  const supabaseUrl = getPublicSupabaseConfig()?.url;
  const hrefFor = (href: string) => resolveCmsHref(href, supabaseUrl);
  if (document) return <div className={preview ? 'cms-renderer is-preview' : 'cms-renderer'}>{document.root.map((section) => <section className={`editor-section variant-${section.variant} width-${section.width} spacing-${section.spacing}`} key={section.id}><div className={`editor-columns columns-${section.columns.length}`}>{section.columns.map((column) => <div className={`editor-column span-${column.width}`} key={column.id}>{column.blocks.map((node) => node.type === 'component_instance' ? (components[node.componentId] ? <BlockRenderer key={node.id} document={components[node.componentId].published_definition} preview={preview} collections={collections} components={components}/> : null) : <BlockRenderer key={node.id} blocks={[contentBlockSchema.parse(node)]} preview={preview} collections={collections} components={components}/>)}</div>)}</div></section>)}</div>;
  return <div className={preview ? 'cms-renderer is-preview' : 'cms-renderer'}>
    {blocks.map((block, index) => {
      ...
      if(block.type==='approved_embed')return <div className={`container approved-embed aspect-${block.aspect.replace(':','-')}`} key={key}><iframe src={block.url} title={block.title} loading="lazy" allow="fullscreen; picture-in-picture" referrerPolicy="strict-origin-when-cross-origin"/></div>;
      return null;
    })}
  </div>;
}
