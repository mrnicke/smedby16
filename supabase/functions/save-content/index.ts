...
const schemas = {
  ...
  news_posts: z.object({ id: uuid.optional(), slug: z.string().regex(/^[a-z0-9-]+$/), title: z.string().min(1).max(160), summary: z.string().max(600).default(''), body_blocks: blockListSchema, hero_media_id: optionalUuid, published_at: z.string().datetime().nullable(), is_published: z.boolean() }),
  calendar_events: z.object({ id: uuid.optional(), title: z.string().min(1).max(160), description: z.string().max(2500).default(''), starts_at: z.string().datetime(), ends_at: z.string().datetime().nullable().optional(), all_day: z.boolean(), location: z.string().max(240).nullable().optional(), category: z.enum(['möte','aktivitet','underhåll','information']), external_url: z.string().url().startsWith('https://').nullable().optional(), is_published: z.boolean() }),
  documents: z.object({ id: uuid.optional(), title: z.string().min(1).max(160), description: z.string().max(1000).default(''), document_date: z.string().date().nullable().optional(), category: z.string().min(1).max(80), media_id: uuid, is_published: z.boolean(), sort_order: z.number().int() }),
