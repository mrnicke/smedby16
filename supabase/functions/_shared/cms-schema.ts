...
// Canonical CMS domain schemas shared by the Astro app and Supabase Edge Functions.
...
export const safeAssetUrlSchema = z.string().trim().refine((value) => {
  if (value.startsWith('/')) return !value.startsWith('//');
  try { return new URL(value).protocol === 'https:'; } catch { return false; }
}, 'Ange en säker intern bildadress eller en https-adress.');

const imageSchema = z.object({
  src: safeAssetUrlSchema,
...
const richTextMarkSchema = z.union([
  ...
  z.object({ type: z.literal('link'), attrs: z.object({ href: safeUrlSchema }).strict() }).strict(),
]);
const richTextNodeSchema: z.ZodType<unknown> = z.lazy(() => z.discriminatedUnion('type', [
  z.object({ type: z.literal('text'), text: z.string().max(20_000), marks: z.array(richTextMarkSchema).max(8).optional() }).strict(),
  z.object({ type: z.literal('paragraph'), content: z.array(richTextNodeSchema).max(250).default([]) }).strict(),
  z.object({ type: z.literal('heading'), attrs: z.object({ level: z.union([z.literal(2), z.literal(3)]) }).strict(), content: z.array(richTextNodeSchema).max(50).default([]) }).strict(),
  z.object({ type: z.literal('bulletList'), content: z.array(richTextNodeSchema).max(100).default([]) }).strict(),
  z.object({ type: z.literal('orderedList'), content: z.array(richTextNodeSchema).max(100).default([]) }).strict(),
  z.object({ type: z.literal('listItem'), content: z.array(richTextNodeSchema).max(50).default([]) }).strict(),
  z.object({ type: z.literal('blockquote'), content: z.array(richTextNodeSchema).max(50).default([]) }).strict(),
...
export const richTextDocumentSchema = z.object({
  ...
  let nodes = 0; let textLength = 0; let maxDepth = 0;
  const visit = (value: unknown, depth: number) => {
    nodes += 1; maxDepth = Math.max(maxDepth, depth);
    if (!value || typeof value !== 'object') return;
    const record = value as { text?: unknown; content?: unknown };
    if (typeof record.text === 'string') textLength += record.text.length;
    if (Array.isArray(record.content)) for (const child of record.content) visit(child, depth + 1);
  };
  for (const node of document.content) visit(node, 1);
  if (maxDepth > 12) context.addIssue({ code: 'custom', message: 'Textdokumentet är för djupt nästlat.' });
  if (nodes > 1_000) context.addIssue({ code: 'custom', message: 'Textdokumentet innehåller för många noder.' });
  if (textLength > 100_000) context.addIssue({ code: 'custom', message: 'Textdokumentet innehåller för mycket text.' });
});
...
export const componentPropertyValueSchema = z.union([
  ...
  z.array(z.union([z.string().max(2_000), z.number().finite(), z.boolean(), z.null()])).max(50),
]);
export const componentPropertyDefinitionsSchema = z.record(
  z.string().regex(/^[a-z][a-z0-9_]{0,63}$/),
  z.object({ label:z.string().trim().min(1).max(120), type:z.enum(['text','number','boolean','url']), required:z.boolean().default(false), defaultValue:componentPropertyValueSchema.optional() }).strict(),
).refine((value) => Object.keys(value).length <= 30, 'För många komponentegenskaper.');
export const componentInstanceSchema = z.object({
  id: z.string().uuid(),
  type: z.literal('component_instance'),
  componentId: z.string().uuid(),
  properties: z.record(z.string().regex(/^[a-z][a-z0-9_]{0,63}$/), componentPropertyValueSchema).refine((value) => Object.keys(value).length <= 30, 'För många instansegenskaper.').default({}),
}).strict();
...
export type ReusableComponentDefinition = { id:string; allowed_instance_properties:unknown; published_definition?:unknown };
export function validateComponentInstances(document: EditorDocumentV2, components: ReusableComponentDefinition[], rootComponentId?:string) {
  const definitions=new Map(components.map((component)=>[component.id,{properties:componentPropertyDefinitionsSchema.parse(component.allowed_instance_properties),document:component.published_definition===undefined?null:editorDocumentV2Schema.parse(component.published_definition)}]));
  const visit=(current:EditorDocumentV2,path:string[],depth:number)=>{
    if(depth>8)throw new Error('Synkade komponenter är nästlade för djupt.');
    for(const section of current.root)for(const column of section.columns)for(const node of column.blocks){
      if(node.type!=='component_instance')continue;
      if(path.includes(node.componentId))throw new Error('En cirkulär referens mellan synkade komponenter är inte tillåten.');
      const definition=definitions.get(node.componentId);if(!definition)throw new Error('En synkad komponent saknas eller är inte publicerad.');
      for(const key of Object.keys(node.properties))if(!definition.properties[key])throw new Error(`Instansegenskapen ${key} är inte tillåten.`);
      for(const [key,rule] of Object.entries(definition.properties)){
        const value=node.properties[key];if(rule.required&&(value===undefined||value===null||value===''))throw new Error(`Instansegenskapen ${key} krävs.`);
        if(value===undefined||value===null)continue;
        const valid=rule.type==='text'?typeof value==='string':rule.type==='number'?typeof value==='number':rule.type==='boolean'?typeof value==='boolean':typeof value==='string'&&safeUrlSchema.safeParse(value).success;
        if(!valid)throw new Error(`Instansegenskapen ${key} har fel typ.`);
      }
      if(definition.document)visit(definition.document,[...path,node.componentId],depth+1);
    }
  };
  visit(document,rootComponentId?[rootComponentId]:[],0);
  return document;
}
